...
class go extends Component {
  ...
  render() {
    ...
    const cellList = ["번호", "이미지", "가게명","평점" ,"최소주문", "배달요금"]
    const filteredComponents = (data) => {
      data = data.filter((c) => {
          return String(c.RESTAURANT_NAME).indexOf(this.state.searchKeyword) > -1 || String(c.STARS).indexOf(this.state.searchKeyword) > -1 
