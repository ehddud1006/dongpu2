    ...
    while True :
        ...
        for item in items :
            ...
            if item.find_element_by_css_selector(".restaurant-name").text == restaurant_names[index] and visit[index]:
                ...
                posting = driver.find_element_by_xpath('//*[@id="content"]/div[2]/div[1]/ul/li[3]/a')
                posting.click()
                ...
                sql3 = "insert into RESTAURANT_INFO (BUSINESS_HOURS, TELEPHONE_NUMBER, ADDRESS, MIN_COST, PAYMENT_METHOD, COMPANY_NAME, BUSINESS_NUMBER, FOOD_ORIGIN, ID) values(%s, %s, %s, %s,%s, %s, %s, %s,%s)"
                cursor.execute(sql3,(business_hours,telephone_number,address,min_cost,payment_method,company_name,business_number,food_origin,id))
                db.commit()
                break
        index +=1    
        driver.back()
